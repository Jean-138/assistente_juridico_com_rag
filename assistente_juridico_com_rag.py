...
st.set_page_config(page_title = "I.A Auxiliar jurídico ", page_icon = ":100:", layout = "wide")
...
with st.sidebar:
    ...
    st.write("1) Informe sua chave no campo acima.\n2) Digite sua pergunta ou dúvida.\n3) Clique em Enviar.\n4) obter a sua chave groq https://console.groq.com/home.")
    st.info("Aviso: a IA pode cometer erros.")

    
# Exibe títulos e informações sobre o projeto
st.title("I.A Auxiliar jurídico ")
st.title("⚖️")
...
def aj_cria_banco_vetorial(pdf_bytes) -> Chroma:
...
if pdf_file and "vectordb_ready" not in st.session_state:
    with st.spinner("Indexando o PDF no ChromaDB…"):
        st.session_state.vectordb = aj_cria_banco_vetorial(pdf_file.read())
...
# acima, a estratégia matemática é busca vetorial por similaridade, mais especificamente, 
...
# Define as instruções principais do programa assistente jurídico
